...
[README](https://github.com/Sythos/JS_Barcode_Universal/blob/main/README.md#supported-barcode-formats).
...
| QR family | `qr`, `microqr`, `rmqr` | Matrix | 3 | 3 | These are related families with different geometry and feature sets. vCard, SPARQCode, Swiss QR-bill and SEPA/EPC QR Code are payload conventions inside an ordinary `qr` symbol, not separate formats — see `docs/formats/qr-family.md`. |
...
| PDF417 family | `pdf417`, `compactpdf417`, `micropdf417` | Stacked | 3 | 3 | Full, truncated and Micro geometry are not aliases. AAMVA DL/ID data is a payload convention on `pdf417`, not a separate format — see `docs/formats/pdf417-family.md`. |
