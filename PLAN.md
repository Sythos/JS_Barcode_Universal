...
46 listed formats write, 45 read; PDF417 image reading is now enabled after synthetic, black-box and
...
separate fixed-grid 2D path, and Codablock-F uses a separate stacked Code 128
path with complete-row validation.
The regression and false-positive corpus includes the baseline suite plus focused coverage for large-raster fallback, strict camera-profile validation, and the M2–M10 format group. The camera profile requires quiet-zone-qualified, repeated 1D reads, emits no partial or structurally inconsistent value, and exposes confidence metadata; it evaluates the fixed in-plane orientations `0°`, `45°`, `90°`, `135°`, `180°`, `225°`, `270°` and `315°` for supported camera detector passes. It deliberately preserves the unrestricted default mode. A local benchmark of
...
- *Moderate, reuses existing 2D machinery:* Code 16K (stacked Code 128).
