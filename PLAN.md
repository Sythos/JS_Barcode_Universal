...
61 listed formats write, 60 read; PDF417 image reading is now enabled after synthetic, black-box and
real-device validation. Extreme glare, severe occlusion, curved media and multi-symbol scenes
remain outside the validated robustness envelope. KarTrak ACI and JAB Code ship separately and
experimentally (see M21 and M24 below): both are colour-coded, not `BitMatrix`-based, so both are
outside this 61/60 count and the `listFormats()` registry.
...
| **1D** — EAN-13/8, UPC-A/E, ISBN, JAN, Code 128, GS1-128, Code 39, Code 93, ITF, ITF-14, Code 25, Industrial/IATA 2 of 5, Data Logic 2 of 5, Matrix 2 of 5, FIM, ITF-6, Codabar, Code 11, MSI, Plessey, Code 32, PZN, Telepen, Pharmacode, the seven postal 4-state formats, PostBar C10/D22/G12 and DX Film Edge Barcode | ✅ 1D suite |
| **1D readers** — EAN-13/8, UPC-A/E, ISBN, JAN, Code 128, GS1-128, Code 39, Code 93, ITF, ITF-14, Code 25, Industrial/IATA 2 of 5, Data Logic 2 of 5, Matrix 2 of 5, FIM, ITF-6, Codabar, Code 11, MSI, Code 32, PZN, Telepen, POSTNET, PLANET, RM4SCC, KIX, Australia Post, Japan Post, IMb, PostBar C10/D22/G12, DX Film Edge Barcode, EAN-2/5 parent-bound supplements and GS1 DataBar physical variants | ✅ phantom-free |
...
| M23 | DX Film Edge Barcode | Kodak's latent-image two-track code printed along 35mm film below the sprocket holes (distinct from the film cartridge's ordinary ITF barcode, already covered by `itf`). Structure verified against US Patent 4,965,628A's own page images; its high-level summary states a 7-bit frame-number field, which conflicts with real film samples and the only known open-source implementation (Zint's `dxfilmedge.c`, consulted as a black-box behavioural reference only) — both agree on 6 bits, which this SDK follows. Fixed-width 6-start/7-product/1-sep/4-generation/[6-frame/1-half-frame/1-sep]/1-parity/4-stop bit layout, XOR parity, fixed clock-track synchronization pattern determined by symbol length. Real `BitMatrix` format (2 fixed rows: clock track, data track), wired into `ONED_FORMATS`/`encode()`/`decode()`/`listFormats()` normally | Round trips with and without frame info; field-range validation; tampered-parity and tampered-clock-track rejection; image-pipeline round trips across multiple scales/margins; camera-profile quiet-zone check; false-positive rejection against random-noise rasters; a real bug (row-band detection assumed exactly one pixel row per module, breaking on any real render scale since the two tracks are adjacent with no gap) found and fixed during implementation, not after |
| M24 | JAB Code (experimental, colour-coded) | Not a `BitMatrix` format, same architectural boundary as KarTrak (M21): full-colour 2D matrix barcode (ISO/IEC 23634:2022, Fraunhofer SIT). Implements only the reference encoder's own "default mode" fast path (8 colours, ECC level 3, mask type 7, no metadata Part I/II) plus byte-mode-only data encoding — read directly from the reference implementation's own C source (github.com/jabcode/jabcode, MIT since April 2026), not a secondary description: default palette, alignment-pattern position tables, the LDPC pseudo-random generator/parity-check-matrix construction/Gauss-Jordan elimination/systematic encoding, hard-decision LDPC decoding, byte-mode segment format, and module placement (finder pattern, alignment patterns, colour-palette swatches, data area). Reuses this SDK's existing `PerspectiveTransform`, `PolychromeMatrix`, `toColorImageData` and `classifyGrid` rather than new geometry/colour primitives. Ships as its own `@sythos/js_barcode_universal/jabcode` subpath, outside `encode()`/`decode()`/`listFormats()` | Round-trips across payload sizes including the byte-mode length-prefix boundary, a payload near a version-32 symbol's real capacity ceiling, and a payload large enough to span multiple LDPC sub-blocks; hard-decision LDPC recovery from deliberately flipped data modules; full raw-image round trip via known-geometry corners at a realistic module scale; no compiler was available to build the reference as a decode oracle, so verification is self-consistency only, not confirmed interop with the real jabcode library — see `docs/JABCODE_NOTES.md`, which also records a real indexing bug (a mis-sized array causing a silent out-of-bounds write on a JS typed array) this process found and fixed during implementation |
...
**Implemented outside the counted 61/60 registry — JAB Code (experimental)** — full-colour 2D matrix barcode (ISO/IEC 23634:2022), same architectural boundary as KarTrak. Ships as its own `@sythos/js_barcode_universal/jabcode` subpath. Implements only the reference encoder's own "default mode" (8 colours, ECC level 3, mask type 7, no metadata Part I/II) plus byte-mode-only data encoding — not the full specification. Encoding and matrix decoding are complete and self-consistency tested; decoding from a raw image requires known symbol corners, like KarTrak, since no detector exists for this format yet; no live reference build was available to confirm interop beyond this module's own round trip. See `docs/formats/jabcode.md` and `docs/JABCODE_NOTES.md`.
